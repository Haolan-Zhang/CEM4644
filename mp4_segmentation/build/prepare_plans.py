...
For every plan listed in aec_seg/config.py this script takes the annotation-aligned image
(F1_scaled.png, drawn at 1 pixel = 1 cm), resamples it to a plan-specific scale so that the scale
...
def parse_svg(svg_text: str) -> dict:
    ...
    for g in root.iter(NS + "g"):
        ...
        if cls.startswith("Space ") and poly is not None:
            p = _poly(poly.get("points"))
            if len(p) < 3:
                continue
            name, dims = None, None
            for t in g.iter(NS + "text"):
                ...
                if m:
                    ...
                    name = s
            raw = cls.split(" ", 1)[1]
            rooms.append({"raw_type": raw, "type": ROOM_TYPES.get(raw, raw.lower()), "label": name or "", "dims_m": dims, "poly": p})
        elif (cls == "Door" or cls.startswith("Door ")) and poly is not None:
            b = _bbox(_poly(poly.get("points")))
            if b: doors.append(b)
        elif cls.startswith("Window") and poly is not None:
            b = _bbox(_poly(poly.get("points")))
            if b: windows.append(b)
        elif cls in ("Wall", "Wall External") and poly is not None:
            w_ = _poly(poly.get("points"))
            if w_.ndim == 2 and len(w_) >= 3: walls.append(w_)
        elif cls.startswith("FixedFurniture") or cls == "Stairs":
            # the symbol's outline is the first polygon below the group (its BoundaryPolygon / Flight child)
            inner = next(g.iter(NS + "polygon"), None)
            tokens = cls.split()
            kind = next((v for k, v in FIXTURES.items() if any(k in tok for tok in tokens)), None)
            b = _bbox(_poly(inner.get("points"))) if (kind and inner is not None) else None
...
def build_plan(z: zipfile.ZipFile, pid: str, factor: float, out_dir: Path, meta: dict) -> dict:
    base = f"cubicasa5k/high_quality_architectural/{pid}/"
    img = Image.open(io.BytesIO(z.read(base + "F1_scaled.png"))).convert("RGB")
    key = parse_svg(z.read(base + "model.svg").decode("utf-8"))
...
def main():
    ...
    ap.add_argument("--zip", required=True, help="cubicasa5k.zip from Zenodo")
    ...
    for key, spec in SETS.items():
        ...
        for pid, meta in spec.plans.items():
            k = build_plan(z, pid, meta["factor"], out_dir, meta)
